<?php
/* =====================================================================
   enroll.php  ·  Activity-08 Group 4  ·  Student enrollment
   ---------------------------------------------------------------------
   Builds on the Activity-03 registration form (mini stepper, password
   strength meter, photo + docs upload) and wires it to the database.

   On submit it validates server-side, uploads the profile photo and the
   school documents into uploads/, then inserts a `users` row
   (account_type='student') plus a `students` row inside a transaction.
   Every user value that reaches SQL uses a prepared statement.
   ===================================================================== */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($missing) {
        ...
        if ($taken) {
            ...
            /* --- File uploads (only if provided) ---
               Validation is inline so the error message stays specific;
               saveUpload() then writes the file to disk with a safe name. */
...
/* --- Helper: move an uploaded file to the uploads dir with a safe,
   unique filename. Returns the saved name, or '' on failure. The caller
   has already validated the file type before calling this. --- */
...
/* --- Helper: student numbers are issued only after a user row exists.
   Format follows the PUP style: YYYY-#####-MN-#. The user id gives us a
   stable starting sequence, and the database check prevents collisions. --- */
...
<body class="enroll-bg">
    ...
    <main class="enroll-shell <?= $ok ? 'success-mode' : '' ?>">
        <!-- ----- SECTION MENU -----
             Why: the form is long, so labelled buttons give keyboard and
             mouse users a quick route to each section. -->
        <?php if (!$ok): ?>
