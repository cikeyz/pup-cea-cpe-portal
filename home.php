<?php
/* =====================================================================
   home.php  ·  Activity-08 Group 4  ·  Logged-in dashboard
   ---------------------------------------------------------------------
   Session-guarded. Pulls the account row from `users` and, depending on
   account_type, the matching `students` or `guests` row so we can show
   a real profile. The avatar is either the enrollment photo (when the
   file really exists in uploads/) or an initials circle built from the
   first letter of the student's first name / the guest's username.
   ===================================================================== */
...
/* Avatar decision: photo only if the enrolled student uploaded one AND
   the file is still on disk. Everyone else gets the initials circle. */
$hasPhoto = false;
...
<body class="dashboard">
    <main class="dash">
        <!-- ----- BANNER -----
             Why: maroon/gold header carries the PUP identity on the
             post-login screen, distinct from the CEA-photo auth pages. -->
        ...
        <section class="dash-body">
            <!-- ----- AVATAR -----
                 Why: enrolled students with an uploaded photo see that
                 photo; guests (and students without one) see a generated
                 initials circle, per the activity brief. -->
